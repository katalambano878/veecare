...
export default function OfflinePage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-brand-cream via-white to-brand-nude/30 flex items-center justify-center px-6">
      <div className="max-w-md w-full text-center">
        {/* Animated illustration */}
        <div className="relative mb-8">
          <div className="w-32 h-32 mx-auto relative">
            {/* Outer ring */}
            <div className="absolute inset-0 rounded-full border-4 border-brand-nude/60 animate-pulse" />
            {/* Inner circle */}
            <div className="absolute inset-3 rounded-full bg-gradient-to-br from-brand-cream to-brand-nude/50 flex items-center justify-center">
              <div className="relative">
                <i className="ri-wifi-off-line text-5xl text-brand-espresso" />
                {/* Animated dots */}
        ...
        <div className="space-y-3 mb-8">
          <button
            onClick={handleRetry}
            disabled={retrying}
            className="w-full bg-brand-espresso hover:bg-brand-cocoa disabled:bg-brand-nude/60 text-white py-4 px-6 rounded-2xl font-semibold transition-all active:scale-[0.98] shadow-lg shadow-brand-espresso/20 flex items-center justify-center gap-2"
          >
            {retrying ? (
              <>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              >
                <i className={`${tip.icon} text-brand-espresso text-sm`} />
                <span>{tip.text}</span>
              </div>
            ))}
          </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          <div className={`w-2 h-2 rounded-full ${isOnline ? 'bg-brand-espresso' : 'bg-red-400'} ${!isOnline && 'animate-pulse'}`} />
