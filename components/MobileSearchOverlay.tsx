...
export default function MobileSearchOverlay({ isOpen, onClose }: MobileSearchOverlayProps) {
  ...
  const searchSuggestions = [
    { name: 'Trending Crossbody Bag', category: 'Bags & Accessories', price: 'GH₵289', image: 'https://readdy.ai/api/search-image?query=luxury%20crossbody%20bag%20on%20white%20surface%20clean%20minimalist%20white%20studio%20background%20professional%20product%20photography%20high%20quality%20detailed%20premium%20fashion&width=80&height=80&seq=mob1&orientation=squarish' },
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 relative">
            <input
              ...
              className="w-full pl-10 pr-10 py-3 bg-gray-100 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-brand-mauve/40"
              autoFocus
            />
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button 
                    ...
                    className="text-xs text-brand-espresso font-medium whitespace-nowrap"
                  >
                    Clear All
                  </button>
                </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {popularSearches.map((search, index) => (
                  <button
                    key={index}
                    className="px-4 py-2 bg-gray-100 text-gray-700 text-sm rounded-full hover:bg-brand-nude/30 hover:text-brand-mauve transition-colors whitespace-nowrap"
                  >
                    {search}
                  </button>
                ))}
              ...
              {searchSuggestions.map((product, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm font-semibold text-brand-espresso mt-1">{product.price}</p>
                  </div>
                </Link>
              ))}
            </div>
